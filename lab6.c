...
int main(int argc, char *argv[])
{
    ...
    for (int k = 0; k < 3; k++){
        ...
        if (row_rank == k)
            MPI_Bcast(matA[cart_rank / 4] + ((cart_rank / 4 + k) % 4), 1, MPI_INT, (cart_rank / 4 + k) % 4, row_comm); //everybody needs to send my_rank, matA[my_rank / 4][my_rank % 4], matB[my_rank / 4][my_rank % 4], row_rank, coord[0],coord[1]);
            
        printf("rank=%d, col=%d, root=%d\n", cart_rank, row_rank, (row_rank + k) % 4);
         
        //~ // Computation
        //~ matC[my_rank / 4][my_rank % 4] += matA[my_rank / 4][my_rank % 4] * matB[my_rank / 4][my_rank % 4];
        ...
        //~ MPI_Sendrecv_replace(matB[my_rank / 4] + (my_rank % 4), 1, MPI_INT,
